...
namespace le0tard.FinalCharacterController
{
    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (_actionsInput._combatState == PlayerCombatState.Blocking)
            {
                HandleBlockMovement();
            }
            else
        ...
        private void HandleBlockMovement()
        {
            ...
            // Movement direction is still camera-relative
            ...
            // 🔑 Force player rotation to face camera forward
            if (cameraForwardXZ.sqrMagnitude > 0.01f)
            {
                //need to add fot/underkropp animation rotate 90 här...
                Quaternion targetRot = Quaternion.LookRotation(cameraForwardXZ, Vector3.up);
                transform.rotation = Quaternion.Slerp(transform.rotation, targetRot, playerModelRotationSpeed * Time.deltaTime);
            }
        }
